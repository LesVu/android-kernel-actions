...
msg "::group::Updating container..."
apt update && apt upgrade -y
msg "Installing essential packages..."
apt-get install -y --no-install-recommends git make bc bison openssl \
...
msg "::group::Installing toolchain..."
if [[ $arch = "arm64" ]]; then
    ...
    if [[ $compiler = gcc/* ]]; then
        ...
        url="https://android.googlesource.com/platform/system/libufdt/+archive/refs/heads/main.tar.gz"
        echo "Downloading $url"
        if ! wget --no-check-certificate "$url" -O /tmp/libufdt.tar.gz &>/dev/null; then
            err "Failed downloading toolchain, refer to the README for details"
            exit 1
        fi

        mkdir -p /aosp-clang /aosp-gcc-arm64 /aosp-gcc-arm /aosp-gcc-host /libufdt
        ...
        extract_tarball /tmp/libufdt.tar.gz /libufdt
...
msg "Setting output file to $image"
