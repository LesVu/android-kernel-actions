...
apt install -y --no-install-recommends git make bc bison openssl \
  ...
  device-tree-compiler ca-certificates python3 python2.7 xz-utils lib32z1
...
if [[ $arch = "arm64" ]]; then
  ...
  if [[ $compiler = gcc/* ]]; then
    ...
    if ! git clone https://android.googlesource.com/platform/prebuilts/gcc/linux-x86/aarch64/aarch64-linux-android-4.9 -b android-6.0.1_r32 --depth=1 /aarch64-linux-android-4.9 &>/dev/null; then
      err "Failed downloading toolchain, refer to the README for details"
      exit 1
    fi
